                        ...
                        <div class="card">
                            ...
                            <div class="card-body" style="overflow-y:scroll;">
                                <table id="miTabla" class="table table-bordered table-striped" style="width: 100%;">
                                    ...
                                    <tbody>
                                        <?php
                                        $query = "SELECT * FROM cupones ORDER BY id DESC";
                                        $query_run = mysqli_query($con, $query);
                                        if (mysqli_num_rows($query_run) > 0) {
                                            foreach ($query_run as $registro) {
                                        ?>
                                                <tr>
                                                    <td><?= $registro['cupon']; ?></td>
                                                    <td><?= $registro['codigo']; ?></td>
                                                    <td><?= $registro['porcentaje']; ?></td>
                                                    <td><?= $registro['maximo']; ?></td>
                                                    <td><?= $registro['minimo']; ?></td>
                                                    <td>
                                                        <?php
                                                    ...
                                                    <td>
                                                        <a href="editarcupon.php?id=<?= $registro['id']; ?>" class="btn btn-warning btn-sm m-1"><i class="bi bi-pencil-square"></i></a>

                                                        <form action="codecupones.php" method="POST" class="d-inline">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">NUEVO CUPÓN</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="codecupones.php" method="POST" class="row" enctype="multipart/form-data">
                        <div class="form-floating col-12 col-md-6 mt-1">
                            <input type="text" class="form-control" name="nombre" id="nombre" placeholder="Nombre" autocomplete="off" required>
                            <label for="nombre">Nombre el cupón</label>
                        </div>
                        ...
                        <div class="form-floating col-12 mt-3">
                            <input type="number" class="form-control" name="porcentaje" id="porcentaje" placeholder="Porcentaje" required>
                            <label for="porcentaje">Porcentaje de descuento</label>
                        </div>

                        <div class="form-floating col-12 mt-3">
                            <input type="number" class="form-control" name="maximo" id="maximo" placeholder="Limite maximo de descuento" required>
                            <label for="maximo">Límite máximo de descuento (Pesos MXN)</label>
                        </div>

                        <div class="form-floating col-12 mt-3">
                            <input type="number" class="form-control" name="minimo" id="minimo" placeholder="Monto minimo de compra" required>
                            <label for="minimo">Monto mínimo de compra (Pesos MXN)</label>
                        </div>

                        <div class="form-floating col-12 mt-3 mb-3">
                            <input type="number" class="form-control" name="canjes" id="canjes" placeholder="Número de veces que se puede canjear el cupón" required>
